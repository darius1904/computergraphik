...
//  aufgabe3
//
//  Created by darius on 06.11.2025.
//
...
/*In loadMatrix, the matrix is passed by reference (MatrixInt&) so the function can modify the original object without making a copy.
In writeMatrix, it is passed as a constant reference (const MatrixInt&) to avoid copying while preventing modification.
This improves performance and ensures data safety.
RAII (Resource Acquisition Is Initialization) means resources are acquired in constructors and released in destructors.
Its main advantage is automatic and exception-safe resource management, preventing memory leaks.*/
